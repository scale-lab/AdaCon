...
from test import test, test_adacon  # import test.py to get mAP after each epoch
from models import *
...
def _load_weights(model, weights, device, result_file):
...
def train(hyp):
    ...
    optimizer = _get_optimizer(model, opt.adam)

    start_epoch = 0
    best_fitness = 0.0
    attempt_download(weights)

    _load_weights(model, weights, device, results_file)
...
def train_adacon(hyp):
    ...
    # Initialize model
    model = AdaConYolo(model_args).to(device)
    print(model)
    optimizer = _get_optimizer(model, opt.adam)

    start_epoch = 0
    best_fitness = 0.0
    losses = [0] * len(clusters)

    # _load_weights(model, weights, device, result_file)
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            # Forward
            preds, bc_out = model(imgs)
            
            for cluster_idx, pred in enumerate(preds):
                branch_targets = map_labels_to_cluster(targets, clusters, class_to_cluster_list, cluster_idx, device)
                
                l, loss_items = compute_loss(pred, branch_targets, model, len(clusters[cluster_idx]))
                losses[cluster_idx] += l
            ...
            # Optimize
            if ni % accumulate == 0:
                losses = [l * batch_size / 64 for l in losses] # scale loss
                model.backward(losses)
                losses = [0 for l in losses]
                optimizer.step()
                optimizer.zero_grad()
                ema.update(model)
        ...
        if not opt.notest or final_epoch:  # Calculate mAP
            is_coco = any([x in data for x in ['coco.data', 'coco2014.data', 'coco2017.data']]) and model.nc == 80
            results, maps = test_adacon(model_args, 
                                        opt.data,
                                        batch_size=1,
                                        imgsz=416,
                                        save_json=True,
        ...
        if save:
            ...
            ckpt['model'] = ema.ema.backbone.state_dict()
            # Save last, best and delete
            torch.save(ckpt, last.replace(".pt", "bkbn.pt"))
            if (best_fitness == fi) and not final_epoch:
                torch.save(ckpt, best.replace(".pt", "bkbn.pt"))

            for i, branch in enumerate(model.branches):
                ckpt['model'] = ema.ema.branches[i].state_dict()
                # Save last, best and delete
                torch.save(ckpt, last.replace(".pt", str(i)+".pt"))
                if (best_fitness == fi) and not final_epoch:
                    torch.save(ckpt, best.replace(".pt", str(i)+".pt"))
            del ckpt
...
if __name__ == '__main__':
    ...
    if opt.adaptive:
        train_adacon(hyp)  # train normally
    else:
